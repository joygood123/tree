...
async function runLocalBuild({ deployId, project, sitesDir, tmpDir, githubToken, emit, onLog }) {
  ...
  // ── Step 6 (server apps only): register and serve via proxy ────────────────
  if (siteType === 'server' && startCmd) {
    log(`\n\x1b[36m━━━ Step 6/6 — Start Server App ━━━\x1b[0m`);
    log(`\x1b[33m[info] siteType=server — site will be served via npm/node process\x1b[0m`);
    log(`\x1b[33m[info] startCmd: ${startCmd}\x1b[0m`);
    log(`\x1b[33m[hint] On a real VPS with PM2: pm2 start "${startCmd}" --name ${project.subdomain}\x1b[0m`);
    log(`\x1b[90m[note] Server app mode not auto-started in local/Render mode.\x1b[0m`);
    log(`\x1b[90m[note] Deploy to Docker VPS to enable persistent server processes.\x1b[0m`);
  }
